...
const explicitLevel = (env?.VITE_LOG_LEVEL as string | undefined)?.toLowerCase();
// Support Wrangler/Workers style `DEBUG` var; any truthy non-"false"/"0" enables debug
const debugFlag = (env?.DEBUG as string | undefined)?.toLowerCase();
const isDebugFlag = Boolean(debugFlag) && debugFlag !== "false" && debugFlag !== "0";
const isDev = Boolean(env?.DEV);
const level: LogLevel = ((globalLevel as LogLevel) || (explicitLevel as LogLevel)) || (isDebugFlag ? "debug" : isDev ? "debug" : "info");
