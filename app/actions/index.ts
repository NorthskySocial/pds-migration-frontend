...
export async function createDestAccount(
  ...
  if (!handle.length) {
    ...
    console.log(
      "Handle available " +
      `${pds_origin}/xrpc/com.atproto.identity.resolveHandle?handle=${handle_dest}`
    );
    ...
    if (!submitted) return { handle_dest_available: handle_available, handle_dest: handle_dest, email_valid: email_valid, password_match: password_match, password_too_short: password_too_short }
      ...
      if (import.meta.env.DEV) {
        ...
        return { token_dest: "Test Token", handle_dest_available: handle_available, handle_dest: handle_dest, email_valid: email_valid, password_match: password_match, password_too_short: password_too_short };
      }
      ...
      if (!did) {
        ...
        return { handle_dest: handle_dest, token_dest: response.data.accessJwt };
